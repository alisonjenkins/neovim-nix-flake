      ...
      grammarPackages =
        if pkgs.stdenv.isLinux then
          ...
          let
            ...
            # Desired grammar names for Darwin. Filtered against the actual
            # builtGrammars set below, since not every grammar name here is
            # guaranteed to exist across nixpkgs revisions (renamed/removed
            # upstream) — filtering avoids "undefined variable" eval errors.
            wantedGrammarNames = [
              "ada"
              "agda"
              "angular"
          ...
          in
          map (n: builtGrammars.${n}) (
            builtins.filter (n: builtins.hasAttr n builtGrammars) wantedGrammarNames
          );
